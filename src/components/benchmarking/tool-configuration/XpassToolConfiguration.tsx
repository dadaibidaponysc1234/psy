...
interface XpassToolConfigurationProps {
  toolId: string
...
export function XpassToolConfiguration({
  toolId,
  config,
  jobId,
  onConfigChange,
  ...
  useEffect(() => {
    ...
    if (
      !populations.find((population) => population.name === activePopulation)
    ) {
  ...
  useEffect(() => {
    ...
    if (
      Object.keys(nextMappings).length !== Object.keys(columnMappings).length
    ) {
      onConfigChange({
        ...config,
      ...
      try {
        ...
        setPreviewState((prev) => ({
          ...
          errors: {
            ...prev.errors,
            [populationName]:
              "Failed to load preview. Check path and try again.",
          },
          loading: { ...prev.loading, [populationName]: false },
        }))
        ...
        >
          ...
          <CollapsibleContent className="space-y-4 px-4 pb-4">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
              {(["bed", "bim", "fam"] as const).map((key) => (
                <div key={key} className="space-y-1">
                  <Label className="text-xs uppercase">
                    {key.toUpperCase()} pattern
                  </Label>
                  <Input
                    value={genotypePatterns[key] || ""}
                    onChange={(event) =>
