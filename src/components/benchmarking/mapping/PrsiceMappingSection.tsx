"use client"

import React from "react"
import { Button } from "@/components/ui/button"
import {
  Card,
  CardContent,
  CardHeader,
  CardTitle,
} from "@/components/ui/card"
import {
  Collapsible,
  CollapsibleContent,
  CollapsibleTrigger,
} from "@/components/ui/collapsible"
import { Input } from "@/components/ui/input"
import { Label } from "@/components/ui/label"
import { ChevronDown, ChevronRight, Users } from "lucide-react"
import { ToolPopulationState } from "@/stores/benchmarking-store"

interface PrsiceMappingSectionProps {
  toolId: string
  populations: ToolPopulationState
  isOpen: boolean
  onToggleOpen: (open: boolean) => void
  onPopulationChange: (
    field: keyof ToolPopulationState,
    value: string
  ) => void
  onSavePopulation: () => void
  mappingGrid: React.ReactNode
}

export function PrsiceMappingSection({
  toolId,
  populations,
  isOpen,
  onToggleOpen,
  onPopulationChange,
  onSavePopulation,
  mappingGrid,
}: PrsiceMappingSectionProps) {
  const hasPopulations =
    Boolean(populations.targetPopulation) &&
    Boolean(populations.sourcePopulation)

  return (
    <>
      <Collapsible open={isOpen} onOpenChange={onToggleOpen}>
        <Card className="border-orange-200 bg-orange-50">
          <CollapsibleTrigger asChild>
            <CardHeader className="flex cursor-pointer items-start transition-colors hover:bg-orange-100/50">
              <div className="flex items-center gap-3">
                <Users className="h-5 w-5 text-orange-600" />
                <CardTitle className="text-orange-900">
                  {hasPopulations
                    ? "Population Configuration (Completed)"
                    : "Population Configuration"}
                </CardTitle>
                {isOpen ? (
                  <ChevronDown className="ml-2 h-4 w-4 text-orange-600" />
                ) : (
                  <ChevronRight className="ml-2 h-4 w-4 text-orange-600" />
                )}
              </div>
            </CardHeader>
          </CollapsibleTrigger>
          <CollapsibleContent>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div className="space-y-2">
                  <Label htmlFor={`targetPopulation-${toolId}`}>
                    Target Population Name
                  </Label>
                  <Input
                    id={`targetPopulation-${toolId}`}
                    placeholder="e.g., AFR, EUR, AMR"
                    value={populations.targetPopulation}
                    onChange={(event) =>
                      onPopulationChange("targetPopulation", event.target.value)
                    }
                  />
                  <p className="text-xs text-muted-foreground">
                    The population you want to predict risk for
                  </p>
                </div>
                <div className="space-y-2">
                  <Label htmlFor={`sourcePopulation-${toolId}`}>
                    Source Population Name
                  </Label>
                  <Input
                    id={`sourcePopulation-${toolId}`}
                    placeholder="e.g., EUR, AFR, AMR"
                    value={populations.sourcePopulation}
                    onChange={(event) =>
                      onPopulationChange("sourcePopulation", event.target.value)
                    }
                  />
                  <p className="text-xs text-muted-foreground">
                    The population used to train the risk model
                  </p>
                </div>
              </div>
              <Button
                onClick={onSavePopulation}
                disabled={
                  !populations.targetPopulation || !populations.sourcePopulation
                }
                className="bg-orange-600 hover:bg-orange-700"
              >
                Save Population Names
              </Button>
            </CardContent>
          </CollapsibleContent>
        </Card>
      </Collapsible>

      {mappingGrid}
    </>
  )
}
