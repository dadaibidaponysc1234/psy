...
interface SdprxPopulationConfigurationProps {
  toolId: string
...
export const SdprxPopulationConfiguration: React.FC<
  SdprxPopulationConfigurationProps
> = ({
  toolId,
  populations,
  isOpen,
          ...
          <CardHeader className="flex cursor-pointer items-start transition-colors hover:bg-orange-100/50">
            <div className="flex items-center gap-3">
              ...
              <CardTitle className="text-orange-900">
                {isCompleted
                  ? "SDPRX Population Configuration (Completed)"
                  : "SDPRX Population Configuration"}
              </CardTitle>
              {isOpen ? (
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div className="space-y-2">
                <Label htmlFor={`sdprx-target-${toolId}`}>Target Population Name</Label>
                <Input
                  id={`sdprx-target-${toolId}`}
                ...
                <p className="text-xs text-muted-foreground">
                  The population you want to evaluate with SDPRX
                </p>
              </div>
              <div className="space-y-2">
                <Label htmlFor={`sdprx-base-${toolId}`}>Base Population Name</Label>
                <Input
                  id={`sdprx-base-${toolId}`}
                ...
                />
                <p className="text-xs text-muted-foreground">
                  Provide the cohort used as the base population for SDPRX
                </p>
              </div>
            </div>
            <Button
